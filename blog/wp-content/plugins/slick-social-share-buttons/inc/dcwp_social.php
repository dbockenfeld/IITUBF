...
';
	...
	function dcssb_inc_stumble($link){
		...
		if($this->get_dcssb_default('incStumble')){
			$button = '<li id="dcssb-stumble" class="'.$classSize.'"><iframe src="http://www.stumbleupon.com/badge/embed/'.$size.'/?url='.$elink.'" scrolling="no" frameborder="0" style="border:none; overflow:hidden; width:50px; height: 60px;" allowTransparency="true"></iframe></li>';
		}
		return $button;
	}
...
';
	...
	function dcssb_inc_delicious($link){
		...
		if($this->get_dcssb_default('incDelicious')){
			...
			$button = '<script type="text/javascript" src="http://delicious-button.googlecode.com/files/jquery.delicious-button-1.1.min.js"></script></li>
			';
 ...
 Delicious
	...
	function dcssb_inc_pinit($link){
		...
		$classSize = $size == 'none' || $size == 'horizontal' ? 'size-small' : 'size-box' ;
		...
		if($this->get_dcssb_default('incPinit')){
			...
			$button = '<li id="dcssb-pinit" class="'.$classSize.'"><a href="http://pinterest.com/pin/create/button/?url='.$elink.'&amp;media='.urlencode($image_default).'&amp;description='.$description.'" class="pin-it-button" count-layout="'.$size.'">Pin It</a>
			';
